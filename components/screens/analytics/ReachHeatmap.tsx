...
import { WEEKDAY_ROW_ORDER, WEEKDAY_SHORT, bestReachWindow, formatBand, formatBandHeader, reachLowSampleRatio } from "@/lib/reach";
...
/** Best-time-to-reach heatmap (4b): weekday × hour connectivity/read rate.
 *  Low confidence (sub-sample-gate) cells are hatched + greyed + explained on
 *  hover so a 100%-of-3 cell never reads as a strong signal — confidence is
 *  conveyed by pattern + text, never color alone. */
export function ReachHeatmap({ reach, isMessage }: { reach: ReachByTimeOfDay; isMessage: boolean }) {
  const rateLabel = isMessage ? "Read rate" : "Connectivity %";
  const bands = reach.bandHours === 1
    ? Array.from({ length: 24 }, (_, hour) => hour)
    : [...new Set(reach.cells.map((c) => c.band))].sort((a, b) => a - b);
  ...
  const subtitle = `${rateLabel} by weekday and hour · times in ${APP_TIMEZONE_LABEL}`;

  if (reach.cells.length === 0) {
    return (
      <ChartCard title="Best time to reach" subtitle={subtitle}>
        <EmptyState icon="CalendarClock" title="Not enough call history for a time pattern" body="We need more connected records across the week to recommend a window." />
      </ChartCard>
    );
  }
    ...
    <ChartCard title="Best time to reach" subtitle={subtitle}>
            ...
            <div className="grid gap-1 mb-1" style={{ gridTemplateColumns: `2.6rem repeat(${bands.length}, minmax(0, 1fr))` }}>
              ...
              {bands.map((b) => (
                <div key={b} className="text-center text-[10px] font-medium text-slate-400 whitespace-nowrap">
                  {formatBandHeader(b, reach.bandHours)}
                </div>
              ))}
            </div>
            {/* rows */}
              ...
              {WEEKDAY_ROW_ORDER.map((weekday) => (
                <div key={weekday} className="grid gap-1 items-center" style={{ gridTemplateColumns: `2.6rem repeat(${bands.length}, minmax(0, 1fr))` }}>
                  ...
                  {bands.map((band) => {
                    ...
                    const bandText = formatBand(band, reach.bandHours);
                    if (!cell || cell.total === 0) {
                      return <div key={band} className="h-9 rounded-md bg-slate-50" title={`${WEEKDAY_SHORT[weekday]} ${bandText} · no records`} />;
                    }
                    ...
                    const label = `${WEEKDAY_SHORT[weekday]} ${bandText} · ${fmtPct(cell.rate)} · ${fmtNum(cell.total)} ${isMessage ? "messages" : "calls"}${low ? " — too few to be reliable" : ""}`;
                    return (
                      <button
                        key={band}
                        title={label}
                        aria-label={label}
                        className="h-9 rounded-md flex items-center justify-center text-[10px] font-semibold tabnum focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-ring)] transition-transform hover:scale-[1.04]"
                        style={{ background: bg }}
                      >
      ...
      {window && !tooSparse ? (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[13px] leading-relaxed text-slate-500 mt-0.5">
              {isMessage ? "Read rate" : "Connectivity"} runs {window.liftPp.toFixed(0)} pts above the selection average here ({fmtPct(window.rate)} vs {fmtPct(window.meanRate)}). Schedule the next run toward this window.
            </p>
          </div>
        </div>
      ) : (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-4 flex items-start gap-2.5 text-[13px] text-slate-500">
          ...
          <span>Not enough connected records across the week to recommend a reliable window yet.</span>
        </div>
      )}
    </ChartCard>
  );
